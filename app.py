...
def plot_price_history(hist_df, ticker_symbol):
    ...
    try:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=hist_df.index, y=hist_df['Close'], name='Price', line=dict(color='cyan', width=2)))
        fig.update_layout(title_text=f"{ticker_symbol} Price", title_x=0.5)
        return make_chart_responsive(fig, height=400)
    except: return None
...
def plot_financial_metrics(income_stmt, cash_flow, ticker_symbol):
    try:
        ...
        fig.update_layout(title_text=f'Financials (M USD): {ticker_symbol}', title_x=0.5)
        return make_chart_responsive(fig, height=600)
    except: return None
...
def analyze_single_stock_financials(ticker_symbol, period="2y"):
    try:
        ...
        with st.spinner(f"Analyzing {ticker_symbol}..."):
            ...
            fig_price = plot_price_history(plot_df, ticker_symbol)
            ...
            fig_tech = make_chart_responsive(fig_tech, height=500)
...
st.title("💰 AI Finanical Analysis Tool & Optimizer")
